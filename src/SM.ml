...
let rec eval env conf prg = match prg with
  ...
  | (p::ps) -> match p, conf with 
    ...
    | BEGIN (_, args, locs), (cs, st, (s, i, o))  -> let rec add_val s1 al vl = 
                                                     ...
                                                     let s', st' = add_val (State.enter s (args @ locs)) args st in
    ...
    | ENTER vars, (cs, st, (s, i, o))             -> eval env (cs, st, (State.push s State.undefined vars, i, o)) ps
...
let rec compile_pattern p l_false inds = 
  match p with 
    | Stmt.Pattern.Wildcard     -> []
    | Stmt.Pattern.Ident x      -> []
     | Stmt.Pattern.Sexp (x, ps) ->
        [DUP] @ List.flatten (List.map (fun x -> [CONST x; CALL (".elem", 2, true)]) inds) @
        (let comp, _ = List.fold_left (fun (a, ind) p -> (a @ compile_pattern p l_false (inds @ [ind]), ind + 1)) ([], 0) ps 
        in [DUP; TAG x; CJMP ("z", l_false); DROP] @ comp)

let rec compile_binding p = 
  match p with
    | Stmt.Pattern.Wildcard     -> [DROP]
    | Stmt.Pattern.Ident x      -> [ST x]
    | Stmt.Pattern.Sexp (x, ps) -> let comp, _ = List.fold_left (fun (a, ind) p -> (a @ [DUP; CONST ind; CALL (".elem", 2, true)] @ compile_binding p, ind + 1)) ([], 0) ps in 
        comp @ [DROP] 
...
let rec compile_stmt s l_end = 
  match s with 
    ...
    | Stmt.Return e          -> (match e with
                                  ...
                                  | None   -> [RET false])
    | Stmt.Case (e, bs)      -> let rec find_branch l_start is_fst l_quit bs = 
                                  (match bs with 
                                    | [] -> []
                                    | b::bs' -> let (p, s) = b in 
                                       ...
                                       let start = if is_fst 
                                                   then [] 
                                                   else [LABEL l_start; DROP] in
                                       start @ compile_pattern p l_false [] @ [ENTER (Stmt.Pattern.vars p)] @ compile_binding p @ compile_stmt s "" @ [LEAVE; JMP l_quit] @ find_branch l_false false l_quit bs')
                                    in
                                let l_quit = (if l_end = "" then label_generator#get_label else l_end) in
                                compile_expr e @ find_branch "" true l_quit bs  @ (if l_end = "" then [LABEL l_quit] else [])
