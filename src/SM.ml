...
let rec eval env config prg = match prg with
  ...
  | (p::ps) -> match p, config with 
    | BINOP op, (cs, y::x::st, (s, i, o))         -> eval env (cs, (Expr.to_func op x y)::st, (s, i, o)) ps
    ...
    | RET _, (cs, st, (s, i, o))                  -> (match cs with
                                                      | []            -> (cs, st, (s, i, o))
                                                      | (p', s')::cs' -> eval env (cs', st, (State.leave s s', i, o)) p')  
    | CALL (name, _, _), (cs, st, (s, i, o))      -> eval env ((ps, s)::cs, st, (s, i, o)) (env#labeled name) 
...
let rec compile_expr e = match e with
  ...
  | Expr.Call (name, args)  -> List.fold_left (fun args' expr -> (compile_expr expr) @ args') [CALL (name, List.length args, true)] args
